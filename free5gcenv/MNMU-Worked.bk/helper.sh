...
lb_veth=veth94b8b3bf  # 要改，lb 的 veth
# lb_veth=test
# real_eth=cni0

# ===== Parameters =====
# ----- Pod -----
ue_name=ueransim-ue-deployment-6dd7985b8f-vj8fl  # 要改，ue pod name
# activate_pod_name=()
activate_pod_name=(  # 要改，要掛的 pod
free5gc-upf-deployment-74d59fbdd-m8xpk
)
...
tc_secton_load_balance=egress_redirect
...
then
  case "${1}" in
    ...
    ping)
      ...
      kubectl exec -it ${ue_name} -- ping -I uesimtun0 -c 3 8.8.8.8
    ;;
    curl)
      # kubectl exec -it ${ue_name} -- apt update
